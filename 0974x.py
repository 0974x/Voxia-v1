...
def check_version():
    ...
    if version.parse(git_ver) > version.parse(__VERSION__):
        logo = f'''
               ...
               NEW VERSION OF Voxia IS AVAILABLE -->  {version.parse(git_ver)}  <--
    ...
    while True:
        try:
            async with session.post(f'https://discord.com/api/v9/guilds/{guild_id}/channels', headers=headers, json={'name': channel_name, 'type': type}) as r:
                if r.status == 429:
                    print(f"\033[90m{datetime.utcnow().strftime(' %H:%M:%S.%f - ')}\x1b[38;5;142mRatelimited, Retrying Again Don't Worry..")
                else:
 ...
 \   Y   /  _ \  \/  /  \__  \  
  \     (  <_> >    <|  |/ __ \_
   \___/ \____/__/\_ \__(____  /
    ...
    """)
    time.sleep(0.0002)
    print(redpink(logo), end='')
    slow_write(Center.XCenter(f"""
0974x ||| .gg/jsk
    """))
    print(Center.XCenter(f"""                                  
                          \033[38;2;255;0;205m╔══════════════════════════════╦═══════════════════════════════╗\033[0m
                          \033[38;2;255;0;180m║   \033[37mVersion: {version.parse(__VERSION__)}  0974x | .gg/jsk
